...
(function () {
	...
	function openModal(modal) {
		...
		// !important so it wins even if the host theme has `.hidden{display:none!important}`.
	...
	function initModals() {
		...
		// Timed consultation popup. Does NOT hard-require samvednaData — falls back to a
		// 12s default so a caching/combine plugin that drops the localized data can't
		// suppress the popup (the click-only modals don't need it, which is why they work).
		...
		if (popup) {
			...
			if (!shown) {
				var popupDelay = ( window.samvednaData && parseInt( samvednaData.popupDelay, 10 ) ) || 12000;
				window.setTimeout(function () {
					openModal(popup);
	...
	function validate(form) {
		...
		if (get('parentName').length < 2) { errors.parentName = 'Enter your name.'; }
		var phone = get('phone').replace(/[\s\-()]/g, '');
		if (!/^\+?[0-9]{10,15}$/.test(phone)) { errors.phone = 'Enter a valid phone number (at least 10 digits).'; }
		if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(get('email'))) { errors.email = 'Enter a valid email address.'; }
		if (get('message').length > 1000) { errors.message = 'Keep the message under 1000 characters.'; }
		return errors;
	}
	...
	/* ───────────────────────── Boot ───────────────────────── */
	function init() {
		// Run each module in isolation so a failure in one (e.g. a GSAP/host-JS
		// conflict on the live site) can't stop the others — in particular the
		// timed consultation popup, which lives in initModals().
		var modules = [
			initSmoothScroll, initReveals, initParallax, initHeroParallax, initDoctorAvatar,
			initNavbar, initMobileMenu, initModals, initJourney, initBlogCarousel,
			initScrollers, initFaq, initForms
		];
		for (var i = 0; i < modules.length; i++) {
			try {
				modules[i]();
			} catch (err) {
				if (window.console && console.error) { console.error('[samvedna] init failed:', modules[i].name, err); }
			}
		}
	}
